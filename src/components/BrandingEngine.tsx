...
export default function BrandingEngine({ brand, setBrand, user }: { brand: any, setBrand: any, user: any }) {
  ...
  if (brand) {
    return (
      <div className="space-y-8 lg:space-y-10">
        <div className="bg-premium-ink text-white p-8 lg:p-12 rounded-[32px] lg:rounded-[40px] relative overflow-hidden shadow-2xl shadow-black/20">
          <div className="relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
              <div className="p-2.5 bg-white/10 rounded-xl backdrop-blur-md w-fit">
                <Sparkles className="w-6 h-6 text-accent-gold" />
              </div>
              <h2 className="text-3xl lg:text-4xl font-extrabold tracking-tight">{brand.name}</h2>
            </div>
            <p className="text-xl lg:text-2xl text-white/50 font-serif italic leading-relaxed max-w-2xl">"{brand.tagline}"</p>
            
            <div className="mt-8 lg:mt-10 flex flex-wrap gap-3 lg:gap-4">
              <div className="px-4 lg:px-5 py-2 lg:py-2.5 bg-white/10 rounded-full text-[12px] lg:text-[13px] font-bold tracking-wide border border-white/10 backdrop-blur-sm">
                <span className="text-white/40 uppercase mr-2 tracking-widest text-[9px] lg:text-[10px]">Archetype</span>
                {brand.archetype}
              </div>
              <div className="px-4 lg:px-5 py-2 lg:py-2.5 bg-white/10 rounded-full text-[12px] lg:text-[13px] font-bold tracking-wide border border-white/10 backdrop-blur-sm">
                <span className="text-white/40 uppercase mr-2 tracking-widest text-[9px] lg:text-[10px]">Personality</span>
                {brand.personality}
              </div>
            </div>
          </div>
                ...
                <div className="flex gap-4">
                  {Object.entries(brand.colors).map(([key, color]: [string, any]) => (
                    <div key={key} className="group relative">
                      <div 
                        className="w-14 h-14 rounded-2xl shadow-sm border border-black/5 transition-transform group-hover:scale-110" 
              ...
              <div>
                ...
                <div className="space-y-3">
                  {brand.content_hooks.slice(0, 3).map((hook: string, i: number) => (
                    <div key={i} className="p-4 bg-gray-50 rounded-2xl text-[13px] border border-black/[0.03] leading-relaxed font-medium">
                      "{hook}"
                    </div>
                  ))}
                </div>
  ...
  return (
    <div className="max-w-4xl mx-auto pb-8 lg:pb-12">
      <div className="text-center space-y-6 mb-12 lg:mb-16">
        <div className="w-20 h-20 lg:w-24 lg:h-24 bg-premium-ink rounded-[28px] lg:rounded-[32px] flex items-center justify-center mx-auto mb-6 lg:mb-8 shadow-2xl shadow-black/20 group hover:scale-105 transition-transform">
          <Sparkles className="text-accent-gold w-8 h-8 lg:w-10 lg:h-10" />
        </div>
        <h2 className="text-3xl lg:text-5xl font-extrabold tracking-tight text-balance leading-tight">Architect your <span className="text-accent-violet italic font-serif font-normal">creator brand</span>.</h2>
        <p className="text-lg lg:text-xl text-premium-muted max-w-2xl mx-auto text-balance leading-relaxed">Define your niche, personality, and vision. Our AI will craft a world-class identity system for your journey.</p>
      </div>

      <div className="premium-card p-6 lg:p-10 bg-white shadow-2xl shadow-black/[0.02]">
        ...
        <textarea 
          ...
          className="w-full h-40 lg:h-48 p-6 lg:p-8 bg-gray-50 rounded-[20px] lg:rounded-[24px] border-none focus:ring-2 focus:ring-accent-violet/20 outline-none text-lg lg:text-xl resize-none font-medium placeholder:text-gray-300 transition-all"
        />
        
        <button 
          onClick={handleGenerate}
          disabled={isLoading || !input.trim()}
          className="w-full mt-6 lg:mt-8 py-4 lg:py-6 bg-premium-ink text-white rounded-[20px] lg:rounded-[24px] font-bold text-lg lg:text-xl flex items-center justify-center gap-4 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:scale-100 shadow-xl shadow-black/10"
        >
          {isLoading ? (
            <>
...
function FeatureItem({ icon: Icon, title, desc }: any) {
  return (
    <div className="flex flex-col items-center text-center space-y-3 group">
      <div className="w-14 h-14 bg-white rounded-2xl border border-premium-border shadow-sm flex items-center justify-center mb-2 group-hover:scale-110 group-hover:shadow-md transition-all">
